...
//RTC_DATA_ATTR unsigned long batch_number = 0; //numéro de lot de mesure depuis le démarrage
int TIME_TO_SLEEP = 10;          // Durée d'endormissement entre 2 cycles complets de mesures (in seconds) par défault 600 (mais rédéfinie par le fichier config)
...
void setup() {
  ...
  // ------------------        HERE IS NEEDED THE PARAMETER FOR THE ENTIER PROGRAMM   ------------------------------------------
      ...
      // Initialize les pin pour les LED et liaison 
      if (led_mode == 1) {
         digitalWrite(greenled, HIGH);
      }
      
      digitalWrite(control_pin, HIGH);              // allume tout les composants
  
      delay(400);                                  // delay pour eviter les ecriture trop rapide sur la carte SD en cas de faux contact à l'allumag et laisser le temps au autre composant (atlas de s'allumer correctement)
    
      // test de présence de carte SD
      test_sd();
      delay(300);
