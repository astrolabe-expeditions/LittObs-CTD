...
void setup() {
  ...
  delay(200);                              // delay pour eviter les ecriture trop rapide sur la carte SD en cas de faux contact à l'allumage

  // initialise les liasons
  if (debug_mode==1) Serial.begin(115200);                        // communication avec le PC
  Wire.begin();                                // for I2C communication

  // eventuel test de présence de capteurs ou composant
  test_bar();
  test_sd();
  ...
  File dataFile = SD.open(filename, FILE_WRITE);       // FILE_APPEND pour esp32, FILE_WRITE pour arduino
...
void loop() {
  ...
  if (debug_mode==1) Serial.print("DATACHAIN COMPLETE:");Serial.println(datachain);
